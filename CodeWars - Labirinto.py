...
# Se o robô chega ao S, o labirinto é validado.
...
while verificador_de_labirinto == 1: 
 ...
 # Função para gerar labirintos aleatóriamente. 
 def cria_labirinto():
  labirinto = []

  lista_espaco_cerquilha = [' ', '#']

  linha_zero = ['#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#']
  labirinto.append(linha_zero)
  linha_um = [random.choice(lista_espaco_cerquilha) for x in range(17)]
  linha_um.insert(0, '#')
  linha_um.insert(18, '#')
  ...
  linha_dois = [random.choice(lista_espaco_cerquilha) for x in range(17)] 
  linha_dois.insert(0, '#')
  linha_dois.insert(19, '#')
  ...
  linha_tres = [random.choice(lista_espaco_cerquilha) for x in range(18)]
  linha_tres.insert(0, '#')
  linha_tres.insert(19, '#')
  labirinto.append(linha_tres)
  linha_quatro = [random.choice(lista_espaco_cerquilha) for x in range(18)]
  linha_quatro.insert(0, '#')
  linha_quatro.insert(19, '#')
  labirinto.append(linha_quatro)
  linha_cinco = [random.choice(lista_espaco_cerquilha) for x in range(18)]
  linha_cinco.insert(0, '#')
  linha_cinco.insert(19, '#')
  labirinto.append(linha_cinco)
  linha_seis = [random.choice(lista_espaco_cerquilha) for x in range(18)]
  linha_seis.insert(0, '#')
  linha_seis.insert(19, '#')
  labirinto.append(linha_seis)
  linha_sete = [random.choice(lista_espaco_cerquilha) for x in range(18)]
  linha_sete.insert(0, '#')
  linha_sete.insert(19, '#')
  labirinto.append(linha_sete)
  linha_oito = [random.choice(lista_espaco_cerquilha) for x in range(18)]
  linha_oito.insert(0, '#')
  linha_oito.insert(19, '#')
 ...
 labirinto_certo = copy.deepcopy(labirinto)
  # copy.deepcopy(x[, memo]) Return a deep copy of x. (Documentação Oficial).
  # Faz uma cópia fiel do mapa validado, sem as marcações do caminho do robô utilizadas no processo.
 
  
  # Regra de movimentação do robô

 # Posição inicial do robô
  # x e y agora está sendo coletado direto do labirinto inicial. Se aleatorizarmos onde 'X' vai aparecer, automaticamente ele já vai encontrar e retornar x e y.
 ...
 #Variáveis para armazenar o local inicial do robô.
 inicio_x = x
 inicio_y = y
...
# Cópia válida e sem movimentos, gerada com a válidação do robô.
